...
- (void)finalizeInit
{
    ...
    if (languageDescription)
    {
        ...
        entry[kMXKLanguagePickerCellDataKeyText] = languageDescription;
        [cellDataArray addObject:entry];
    }
    ...
    for (NSString *language in localizations)
    {
        ...
        if (![language isEqualToString:defaultLanguage])
        {
            ...
            if (languageDescription)
            {
                NSMutableDictionary *entry = [NSMutableDictionary dictionary];
                entry[kMXKLanguagePickerCellDataKeyText] = languageDescription;
                if (localisedLanguageDescription)
                {
                    entry[kMXKLanguagePickerCellDataKeyDetailText] = localisedLanguageDescription;
                }
                if (language)
                {
                    entry[kMXKLanguagePickerCellDataKeyLanguage] = language;
                }
                [cellDataArray addObject:entry];
            }
