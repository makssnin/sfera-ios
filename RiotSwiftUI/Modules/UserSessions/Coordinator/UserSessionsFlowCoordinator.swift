...
final class UserSessionsFlowCoordinator: NSObject, Coordinator, Presentable {
    ...
    init(parameters: UserSessionsFlowCoordinatorParameters) {
        ...
        // indicatorPresenter will be created lazily when needed
    }

    // MARK: - Private

    private func ensureIndicatorPresenter() {
    ...
    private func startLoading() {
        ensureIndicatorPresenter()
        guard let presenter = indicatorPresenter else {
            MXLog.warning("[UserSessionsFlowCoordinator] indicatorPresenter is nil")
            return
        }
        loadingIndicator = presenter.present(.loading(label: VectorL10n.loading, isInteractionBlocking: true))
    }
